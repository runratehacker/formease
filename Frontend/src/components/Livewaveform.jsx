...
import { cn } from '../utils/waveformUtils'
    ...
    const animate = (currentTime) => {
      ...
      if (mode === "static") {
        ...
        for (let i = 0; i < barCount && i < dataToRender.length; i++) {
          ...
          const x = i * step
          // Apply an exponent (1.6) to increase contrast: small values get much smaller, large values stay large
          const barHeight = Math.max(baseBarHeight, Math.pow(value, 1.6) * rect.height * 0.8)
        ...
        for (let i = 0; i < barCount && i < historyRef.current.length; i++) {
          ...
          // Apply the same exponent here for scrolling mode
          const barHeight = Math.max(baseBarHeight, Math.pow(value, 1.6) * rect.height * 0.8)
          const y = centerY - barHeight / 2
